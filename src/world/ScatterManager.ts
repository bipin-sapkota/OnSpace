...
import { FloraLibrary, VARIANTS } from './FloraLibrary';
...
interface ScatterGroup {
  node: QuadNode;
  group: THREE.Group;
  meshes: { mesh: THREE.InstancedMesh; type: ScatterType }[];
...
export class ScatterManager {
  ...
  constructor(params: TerrainParams, archetype: string, lu: PlanetLightUniforms, parent: THREE.Object3D, depleted: Set<string>) {
    ...
    for (let t = 0; t < SCATTER_TYPE_COUNT; t++) {
      for (const g of this.library.geometries[t]) {
        ...
        const arr = new Float32Array(n).fill(this.library.info[t].sway * 25);
        g.setAttribute('aSway', new THREE.BufferAttribute(arr, 1));
      }
    }
  }
  ...
  add(node: QuadNode, data: Float32Array, center: THREE.Vector3): void {
    ...
    for (let b = 0; b < buckets.length; b++) {
      ...
      const mesh = new THREE.InstancedMesh(this.library.geometries[type][variant], this.material, list.length);
      mesh.castShadow = type === ScatterType.Tree || type === ScatterType.Boulder || type === ScatterType.Node;
      ...
      for (let k = 0; k < list.length; k++) {
        ...
        mesh.setMatrixAt(k, dep ? ZERO : mat);
  ...
  update(camLocal: THREE.Vector3): void {
    for (const g of this.groups.values()) {
      ...
      for (const m of g.meshes) m.mesh.visible = d < this.library.info[m.type].maxDist;
    }
  }
  ...
  *nearby(p: THREE.Vector3, radius: number): Generator<ScatterRecord> {
  ...
  deplete(rec: ScatterRecord): void {
    ...
    rec.mesh.setMatrixAt(rec.index, ZERO);
